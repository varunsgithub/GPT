...
Trained on Alpaca GPT-4 dataset to follow instructions. The process follows these steps:

1. **Create a clean dataset** - Prepare instruction-response pairs in consistent format
2. **Develop tokenization function** - Collates instructions and responses with proper formatting
3. **Add padding with ignore index** - Use `-100` for padding tokens to exclude them from cross-entropy loss calculation
4. **Mask instructions (optional)** - Prevent overfitting by masking instruction tokens during loss computation
5. **Train for 1 epoch** - Standard practice for instruction tuning to avoid overfitting

> **Reference**: This approach follows the 2024 paper by Shi et al. ["Instruction Tuning with Loss over Instructions"](https://arxiv.org/abs/2405.14394)
